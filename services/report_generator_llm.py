...
def _resolve_llm_polish_cache_path() -> Path:
    override = os.getenv("NEWS_LLM_POLISH_CACHE_PATH", "").strip()
    if override:
        return Path(override).expanduser().resolve()
    return Path(__file__).parent.parent / "data" / "llm_polish_cache.json"


REPORT_DIR = _resolve_report_dir()
TRANSLATION_CACHE_PATH = _resolve_translation_cache_path()
LLM_POLISH_CACHE_PATH = _resolve_llm_polish_cache_path()
...
class ReportGenerator:
    ...
    def __init__(self):
        ...
        self._llm_polish_cache: Dict[str, Dict[str, str]] = self._load_llm_polish_cache()
        self._llm_polish_failures = 0
        self._llm_polish_disabled_for_run = False
        self._llm_polish_stats = {
            "success": 0,
            "cache": 0,
            "failed": 0,
            "fallback": 0,
    ...
    def _load_llm_polish_cache(self) -> Dict[str, Dict[str, str]]:
        try:
            if LLM_POLISH_CACHE_PATH.exists():
                with open(LLM_POLISH_CACHE_PATH, 'r', encoding='utf-8') as f:
                    data = json.load(f)
            ...
            logger.debug("Failed to load LLM polish cache: %s", exc)
        return {}

    def _save_llm_polish_cache(self) -> None:
        try:
            LLM_POLISH_CACHE_PATH.parent.mkdir(parents=True, exist_ok=True)
            with open(LLM_POLISH_CACHE_PATH, 'w', encoding='utf-8') as f:
                json.dump(self._llm_polish_cache, f, ensure_ascii=False, indent=2)
        except Exception as exc:
            logger.debug("Failed to save LLM polish cache: %s", exc)

    def _llm_polish_enabled(self) -> bool:
        return os.getenv("NEWS_LLM_POLISH", "0").strip().lower() in {"1", "true", "yes", "on"}

    def _llm_broad_recall_enabled(self) -> bool:
        return os.getenv("NEWS_LLM_BROAD_RECALL", "1").strip().lower() in {"1", "true", "yes", "on"}
    ...
    def _extract_json_object(self, text: str) -> Dict[str, str]:
        ...
        if not isinstance(parsed, dict):
            raise ValueError("LLM polish did not return a JSON object")
        return {
            "title": self._normalize_summary_text(str(parsed.get("title", ""))),
            "summary": self._normalize_summary_text(str(parsed.get("summary", ""))),
        }

    def _polish_article_with_llm(
        self,
        item: Dict[str, Any],
        draft_title: str,
        draft_summary: str,
        ...
        if not api_key:
            if not self._llm_polish_disabled_for_run:
                logger.info("LLM polish skipped: NEWS_LLM_API_KEY/OPENAI_API_KEY is not set")
            self._llm_polish_disabled_for_run = True
            return None

        cache_key = f"{self._summary_cache_key(item)}::{max_len}::polish"
        cached = self._llm_polish_cache.get(cache_key)
        if cached:
            self._llm_polish_stats["cache"] += 1
            return cached

        source_text = self._truncate_summary_source_for_ai(
            self._extract_summary_source(item),
            max_chars=self._env_int("NEWS_LLM_POLISH_INPUT_CHARS", 1800),
        )
        if not source_text:
        ...
        prompt = (
            "请把下面这条新闻改写成中文行业周报条目。"
            "只根据原文信息和已有草稿，不要编造，不要补充原文没有的公司、数字或结论。"
            "标题控制在20到35个汉字，简洁、有信息量。"
            f"摘要控制在{max_len}个汉字以内，2到4句；原文信息较多时可分两段，但不要超过字数。"
            "第一段说明发生了什么，第二段保留关键数字、背景和可由原文支持的产业影响。"
            "语言要自然、克制、像正式周报，不要写“该报道围绕”“本文介绍”“建议查看原文”。"
            "删除网页导航、注册登录、广告话术、第一人称和重复标题。"
            "如果信息不足，就写可确认事实，宁可短，不要凑字；不得新增原文没有的公司、数字、时间、客户、金额、产能或结论。"
            "请严格返回 JSON：{\"title\":\"...\",\"summary\":\"...\"}。"
        )
        payload = {
            "model": os.getenv("NEWS_LLM_MODEL", os.getenv("OPENAI_SUMMARY_MODEL", "gpt-4o-mini")).strip() or "gpt-4o-mini",
            "messages": [
                {"role": "system", "content": "你是严谨的中文行业周报编辑，擅长把抓取文本改写得简洁、可信、可读。"},
                {
                    "role": "user",
                    "content": (
                        f"{prompt}\n\n"
                        f"原标题：{self._normalize_summary_text(item.get('title', ''))}\n"
                        f"来源：{self._normalize_summary_text(item.get('source', ''))}\n"
                        f"时间：{self._fmt_time(item.get('publishTime'))}\n"
                        f"草稿标题：{draft_title}\n"
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if not choices:
                    raise ValueError("LLM polish returned no choices")
                result = self._extract_json_object(choices[0].get("message", {}).get("content", ""))
                title = result.get("title") or draft_title
                summary = result.get("summary") or draft_summary
                summary = self._finalize_report_summary(item, summary, max_len=max_len)
                if not self._is_acceptable_report_summary(item, summary, max_len=max_len):
                    self._llm_polish_stats["rejected"] += 1
                    logger.warning("LLM polish rejected by quality gate: %s", self._normalize_summary_text(item.get('title', ''))[:80])
                    return None
                polished = {"title": title, "summary": summary}
                self._llm_polish_cache[cache_key] = polished
                self._save_llm_polish_cache()
                self._llm_polish_stats["success"] += 1
                logger.info("LLM polish success: %s", self._normalize_summary_text(item.get('title', ''))[:80])
                return polished
            except Exception as exc:
                last_exc = exc
                if attempt < retries:
                    ...
                    logger.warning(
                        "LLM polish failed (%s/%s) for current article: %s; retrying in %ss",
                        attempt,
                        retries,
                        exc,
                        delay,
    ...
    def _display_title(self, title: str) -> str:
        ...
        if self._is_mostly_chinese(normalized):
            return self._fix_display_title_translation(normalized, normalized)
        if normalized in TITLE_TRANSLATIONS:
            return self._fix_display_title_translation(TITLE_TRANSLATIONS[normalized], normalized)
        fallback_first = self._fallback_translate_title(normalized)
        if fallback_first != normalized and self._is_mostly_chinese(fallback_first):
            return self._fix_display_title_translation(fallback_first, normalized)
        translated = self._machine_translate_title(normalized)
        if translated:
            return self._fix_display_title_translation(translated, normalized)
        return self._fix_display_title_translation(self._fallback_translate_title(normalized), normalized)

    def _fix_display_title_translation(self, title: str, raw_title: str = '') -> str:
        """Fix machine-translation mistakes in display/report titles."""
        title = self._normalize_summary_text(title)
        raw_title = self._normalize_summary_text(raw_title)
        combined = f"{raw_title} {title}"

        if re.search(r'\bmonitor[s]?\b|\bdisplay[s]?\b|OLED|LCD|Micro[- ]?LED|Mini[- ]?LED', combined, flags=re.IGNORECASE):
            title = re.sub(r'(\d{4})\s*个评论', r'\1 年测评', title)
            title = re.sub(r'(\d{4})\s*条评论', r'\1 年测评', title)
            title = re.sub(r'(\d{4})\s*评论', r'\1 年测评', title)
            title = re.sub(r'\bReviews?\b', '测评', title, flags=re.IGNORECASE)
            title = title.replace('显示器评论', '显示器测评')
            title = title.replace('显示评论', '显示器测评')
            title = title.replace('监视器评论', '显示器测评')
            title = title.replace('最佳27英寸监视器', '最佳 27 英寸显示器')
            title = title.replace('最佳 27 英寸监视器', '最佳 27 英寸显示器')
            title = title.replace('Portable 显示器', '便携式显示器')
            title = title.replace('Gaming 显示器', '游戏显示器')
            title = title.replace('OLED 显示器', 'OLED 显示器')

        title = re.sub(r'\s+', ' ', title).strip()
        title = re.sub(r'(\d{4})\s+年', r'\1年', title)
        title = re.sub(r'(\d+)\s+英寸', r'\1 英寸', title)
        return title
    ...
    def _fallback_translate_title(self, title: str) -> str:
        ...
        rules = [
            (r'^The Best ([\d.]+)[ -]?Inch Monitors? \((\d{4}) Reviews?\)$', r'\2 年最佳 \1 英寸显示器测评'),
            (r'^Best ([\d.]+)[ -]?Inch Monitors? \((\d{4}) Reviews?\)$', r'\2 年最佳 \1 英寸显示器测评'),
            (r'^The Best (.+) Monitors? \((\d{4}) Reviews?\)$', r'\2 年最佳 \1 显示器测评'),
            (r'^Best (.+) Monitors? \((\d{4}) Reviews?\)$', r'\2 年最佳 \1 显示器测评'),
            (r'^(.+) \((\d{4}) Reviews?\)$', r'\2 年 \1 测评'),
    ...
    def generate_daily_report(
        ...
        self._llm_polish_cache = self._load_llm_polish_cache()
        self._llm_polish_failures = 0
        self._llm_polish_disabled_for_run = False
        self._llm_polish_stats = {
            "success": 0,
        ...
        if is_weekly:
            ...
            prefix = f"{report_prefix}_llm_weekly_report" if report_prefix else ''
            if not prefix:
                if is_commercial_space:
                    prefix = 'commercial_space_llm_weekly_report'
                elif is_ai:
                    prefix = 'ai_llm_weekly_report'
                else:
                    prefix = 'llm_weekly_report'
        ...
        logger.info(
            "LLM polish stats: success=%s cache=%s failed=%s rejected=%s fallback=%s forced_compress=%s",
            self._llm_polish_stats.get("success", 0),
            self._llm_polish_stats.get("cache", 0),
            self._llm_polish_stats.get("failed", 0),
            self._llm_polish_stats.get("rejected", 0),
            self._llm_polish_stats.get("fallback", 0),
            self._llm_polish_stats.get("forced_compress", 0),
        )
...
<div class="container">
  ...
  <div class="footer">
    ...
    def _clean_summary_noise(self, text: str) -> str:
        ...
        text = re.sub(r'\s+\d+\s+\[(?:新闻|独家专访|产品|快讯|专栏)\][^。！？!?]*(?:\s+\d+\s+\[(?:新闻|独家专访|产品|快讯|专栏)\].*)?$', ' ', text)
        text = re.sub(
        ...
        text = re.sub(r'\s*(?:上个月|今天|昨日|近日)\s+今天(?:韩国|中国|美国|日本|台湾)?(?:的)?(?:一份)?(?:报告|消息)?(?:称|显示)?[，,]?\s*', ' ', text)
        text = re.sub(r'\s*(?:上个月|今天|昨日|近日)\s+(?=(?:韩国|中国|美国|日本|台湾)?(?:一份)?(?:报告|消息)(?:称|显示))', ' ', text)
    ...
    def _is_llm_recall_candidate(self, item: Dict[str, Any]) -> bool:
        """A wider candidate gate for LLM reports.

        The original script keeps a stricter final-report filter. In this copied
        LLM generator we keep more potentially useful articles, then rely on
        scoring, board limits, and the LLM polish step to make the final text
        readable instead of losing useful context too early.
        """
        title = self._normalize_summary_text(item.get('title', ''))
        if not title:
            return False
        source_text = self._clean_summary_noise(
            f"{item.get('summary', '')} {item.get('content', '')}"
        )
        combined = f"{title} {source_text}".lower()
        if self._has_evergreen_marker(combined):
            return False
        if self._looks_like_navigation_text(combined):
            return False
        return True
    ...
    def _report_summary_max_len(self, item: Dict[str, Any]) -> int:
        """Limit only the final report text; keep crawled content untouched."""
        text = self._normalize_summary_text(
            ' '.join(str(item.get(k, '')) for k in ('title', 'summary', 'content', 'source', 'parent_board', 'board_name'))
        )
        source = self._normalize_summary_text(item.get('source', ''))
        base = self._env_int("NEWS_LLM_REPORT_SUMMARY_MAX_CHARS", 320)

        if re.search(r'价格|供需|趋势|预测|出货|营收|利润|财报|产能|量产|投资|融资|并购|market|price|forecast|revenue|capacity', text, flags=re.IGNORECASE):
            base = max(base, 420)
        if re.search(r'报告|研究|白皮书|深度|分析|年报|季报|panel price|market report', text, flags=re.IGNORECASE):
            base = max(base, 460)
        if re.search(r'会议|展会|活动|邀请|webinar|festival|conference|expo', text, flags=re.IGNORECASE):
            base = min(base, 220)
        if source.lower() in {'ofweek'} or 'ofweek' in source.lower():
            base = min(base, 420)

        return max(180, min(base, self._env_int("NEWS_LLM_REPORT_SUMMARY_HARD_MAX_CHARS", 520)))

    def _summary_has_web_noise(self, text: str) -> bool:
        noise_patterns = [
            r'登录\s*注册', r'查看原文', r'点击阅读', r'免责声明', r'广告', r'联系我们',
            r'首页\s*>', r'时事通讯', r'相关新闻', r'更多信息', r'您可以在此处',
            r'要阅读整篇文章', r'请注册', r'data-react-helmet', r'nocache', r'<meta',
            r'老规矩', r'老师.*带大家', r'不多说了', r'就在昨天', r'我们不知道',
            r'很高兴看到', r'\.\.\.', r'…{2,}',
            r'\d+\s+\[(?:新闻|独家专访|产品|快讯|专栏)\]',
        ]
        return any(re.search(pattern, text, flags=re.IGNORECASE) for pattern in noise_patterns)

    def _finalize_report_summary(self, item: Dict[str, Any], summary: str, max_len: int) -> str:
        summary = self._clean_summary_noise(summary)
        if not summary:
            return ''
        hard_max = max(max_len, 180)
        if len(summary) > hard_max or self._summary_has_web_noise(summary):
            self._llm_polish_stats["forced_compress"] += 1
            summary = self._compress_summary_locally(summary, max_chars=hard_max)
        else:
            summary = self._finalize_summary_text(summary)
        if len(summary) > hard_max:
            summary = self._finalize_summary_text(summary[:hard_max - 1].rstrip('，,;； '))
        return summary

    def _is_acceptable_report_summary(self, item: Dict[str, Any], summary: str, max_len: int) -> bool:
        summary = self._normalize_summary_text(summary)
        if not summary:
            return False
        if len(summary) > max(max_len, 180):
            return False
        if self._summary_has_web_noise(summary):
            return False
        title = self._normalize_summary_text(item.get('title', ''))
        if title and summary.startswith(title) and len(summary) <= len(title) + 8:
            return False
        # A soft fact lock: reject summaries that introduce many numeric claims
        # not present in the original crawled text. This avoids pretty but unsafe hallucinations.
        source_text = self._normalize_summary_text(
            ' '.join(str(item.get(k, '')) for k in ('title', 'summary', 'content'))
        )
        number_pattern = r'\d+(?:\.\d+)?\s?(?:%|亿美元|亿元|万美元|万元|英寸|尼特|万片|万台|GW|MW|GB|B|M|PPI)?'
        src_numbers = set(re.findall(number_pattern, source_text, flags=re.IGNORECASE))
        out_numbers = set(re.findall(number_pattern, summary, flags=re.IGNORECASE))
        src_numeric_tokens = set(re.findall(r'\d+(?:\.\d+)?', source_text))
        unknown_numbers = set()
        for number in out_numbers:
            numeric = re.search(r'\d+(?:\.\d+)?', number)
            if not number or number in src_numbers:
                continue
            if numeric and numeric.group(0) in src_numeric_tokens:
                continue
            unknown_numbers.add(number)
        return len(unknown_numbers) <= 1
    ...
    def _article_summary(self, item: Dict[str, Any], max_len: int = 90) -> str:
        ...
        if cached:
            return self._finalize_report_summary(item, cached, max_len=max_len)
        ...
        if translated_summary:
            ...
            polished = self._polish_article_with_llm(item, self._article_display_title(item), local_summary, max_len)
            if polished:
                self._article_title_cache[key] = polished.get("title") or self._article_display_title(item)
    ...
    def _generate_a1_a8_news_digest(self, all_news: List[Dict[str, Any]]) -> str:
        ...
        broad_recall = self._llm_broad_recall_enabled()
        min_items = self._env_int("NEWS_LLM_BOARD_MIN_ITEMS", 2)
        max_items = self._env_int(
            "NEWS_LLM_BOARD_MAX_ITEMS",
            self._env_int("NEWS_BOARD_MAX_ITEMS", 5),
        )
        if max_items < min_items:
            max_items = min_items
        default_candidate_items = max(max_items * 5, 24) if broad_recall else max(max_items * 2, 8)
        candidate_items = self._env_int(
            "NEWS_LLM_BOARD_CANDIDATE_ITEMS",
            self._env_int("NEWS_BOARD_CANDIDATE_ITEMS", default_candidate_items),
        )
        ...
        board_diagnostics: Dict[str, Dict[str, Any]] = {}
        ...
        for board_name, _ in BOARD_ORDER:
            ...
            eligible_candidates = (
                [item for item in board_candidates if self._is_llm_recall_candidate(item)]
                if broad_recall
                else [item for item in board_candidates if self._is_report_worthy_article(item)]
            )
            ...
            ranked_by_board[board_name] = ranked_candidates
            shown_items = ranked_candidates[:max_items]
            selected_by_board[board_name] = shown_items
            board_diagnostics[board_name] = {
                "classified_count": len(parent_groups.get(board_name, [])),
        ...
        if len(selected_items) > max_total_items:
            ...
            # Preserve the report shape first: keep up to the per-board minimum
            # before filling remaining slots. Otherwise dense boards can make
            # smaller but valid sections vanish.
            for board_name, _ in BOARD_ORDER:
                board_items = selected_by_board.get(board_name, [])
                if not board_items or len(kept_items) >= max_total_items:
                    continue
                for item in board_items[:min_items]:
                    if len(kept_items) >= max_total_items:
                        break
                    key = self._summary_cache_key(item)
            ...
            for board_name in list(selected_by_board.keys()):
                ...
                if board_name in board_diagnostics:
                    board_diagnostics[board_name]["shown_count"] = len(selected_by_board[board_name])
            selected_items = kept_items

        self._batch_summarize_articles(selected_items, max_chars=self._env_int("NEWS_LLM_BATCH_SUMMARY_MAX_CHARS", 420))
        ...
        for board_name, _ in BOARD_ORDER:
            ...
            for item in shown_items:
                ...
                pub_time = self._fmt_time(item.get('publishTime'))
                summary_max_len = self._report_summary_max_len(item)
                summary = self._article_summary(item, summary_max_len)
    ...
    def _generate_board_coverage_diagnostics(
        self,
        board_diagnostics: Dict[str, Dict[str, Any]],
        min_items: int,
        max_items: int,
        ...
        rows = []
        for board_name, _ in BOARD_ORDER:
            stats = board_diagnostics.get(board_name, {})
            classified_count = int(stats.get("classified_count", 0) or 0)
            candidate_count = int(stats.get("candidate_count", 0) or 0)
            eligible_count = int(stats.get("eligible_count", 0) or 0)
            ranked_count = int(stats.get("ranked_count", 0) or 0)
            shown_count = int(stats.get("shown_count", 0) or 0)
            if shown_count >= min_items:
                continue
            if classified_count <= 0:
                reason = "本周未分到该板块，优先检查新闻源覆盖或主题关键词。"
            elif candidate_count <= 0:
                reason = "有板块分类但未进入候选池，建议提高候选池或检查排序。"
            elif ranked_count <= 0:
                reason = "候选存在但被低质量、导航页或长期指南过滤。"
            elif shown_count < min_items:
                reason = "有效候选不足，可能是本周该方向新闻较少或来源覆盖不足。"
            else:
                reason = "已达到展示要求。"
            rows.append(
                f"| {board_name} | {classified_count} | {candidate_count} | "
                f"{eligible_count} | {ranked_count} | {shown_count}/{max_items} | {reason} |"
            )
        ...
        md = [
            "\n## 板块覆盖诊断\n",
            f"> 策略：每个板块目标至少 {min_items} 条，最多 {max_items} 条；不足时不硬凑，标注原因便于补数据源。\n",
            "| 板块 | 分类数 | 候选数 | 初筛可用 | 排名可用 | 展示 | 判断 |",
            "|---|---:|---:|---:|---:|---:|---|",
            *rows,
            "",
        ]
