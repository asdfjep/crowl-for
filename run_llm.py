...
Usage:
  python run_llm.py --topic ai
  python run_llm.py --topic commercial_space
  python run_llm.py --topic display_polarizer
"""
import argparse
import json
import logging
...
def _load_local_llm_config():
    """Load machine-local LLM defaults without hardcoding secrets in source."""
    ...
    try:
        ...
        print(f"Warning: failed to load {config_path}: {exc}", file=sys.stderr)
        return
    env_map = {
...
os.environ.setdefault("NEWS_SUMMARY_MODE", "local")
os.environ.setdefault("NEWS_SUMMARY_USE_AI", "0")
os.environ.setdefault("NEWS_LLM_POLISH", "1")
os.environ.setdefault("NEWS_LLM_BROAD_RECALL", "1")
os.environ.setdefault("NEWS_LLM_BOARD_MIN_ITEMS", "2")
os.environ.setdefault("NEWS_LLM_BOARD_MAX_ITEMS", "5")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run a configured weekly news analyzer with LLM text polish")
    parser.add_argument(
        "--topic",
