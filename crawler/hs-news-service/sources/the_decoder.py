...
class TheDecoderCrawler(BaseCrawler):
    def __init__(self):
        super().__init__(name="the_decoder", base_url="https://the-decoder.com", category="ai")
        self.rss_url = "https://the-decoder.com/feed/"
    ...
    async def fetch_news(self) -> List[NewsItem]:
        ...
        try:
            ...
            logger.error(f"The Decoder 失败: {e}")
        news_list.sort(key=lambda x: x.publish_time or datetime.min, reverse=True)
        logger.info(f"The Decoder：{len(news_list)} 条")
        return news_list
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                news_list.append(NewsItem(
                    title=title, url=link, source="The Decoder",
            ...
            logger.error(f"The Decoder RSS 解析失败: {e}")
        return news_list
