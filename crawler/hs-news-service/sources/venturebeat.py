...
class VentureBeatCrawler(BaseCrawler):
    def __init__(self):
        super().__init__(name="venturebeat", base_url="https://venturebeat.com", category="ai")
    ...
    async def fetch_news(self) -> List[NewsItem]:
        ...
        try:
            ...
            logger.error(f"VentureBeat 失败: {e}")
        news_list.sort(key=lambda x: x.publish_time or datetime.min, reverse=True)
        logger.info(f"VentureBeat：{len(news_list)} 条")
        return news_list
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                news_list.append(NewsItem(
                    title=title, url=link, source="VentureBeat",
            ...
            logger.error(f"VentureBeat RSS 解析失败: {e}")
        return news_list
