...
class EUAICrawler(BaseCrawler):
    def __init__(self):
        super().__init__(name="eu_ai", base_url="https://digital-strategy.ec.europa.eu", category="ai")
        self.rss_url = "https://digital-strategy.ec.europa.eu/en/rss.xml"
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                news_list.append(NewsItem(
                    title=title, url=link, source="EU Digital Strategy",
                    publish_time=publish_time, category="ai",
                    summary=description, location="欧洲",
                ))
                if len(news_list) >= 20:
                    break
        except ElementTree.ParseError as e:
            logger.error(f"EU AI RSS 解析失败: {e}")
        return news_list
