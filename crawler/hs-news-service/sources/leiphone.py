...
class LeiphoneCrawler(BaseCrawler):
    def __init__(self):
        super().__init__(name="leiphone", base_url="https://www.leiphone.com", category="ai")
        self.rss_url = "https://www.leiphone.com/feed"
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                news_list.append(NewsItem(
                    title=title, url=link, source="雷锋网",
                    publish_time=publish_time, category="ai",
                    summary=description, location="中国",
                ))
                if len(news_list) >= 20:
                    break
        except ElementTree.ParseError as e:
            logger.error(f"雷锋网 RSS 解析失败: {e}")
        return news_list
